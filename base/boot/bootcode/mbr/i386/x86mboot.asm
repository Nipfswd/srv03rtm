        page    ,132
;
;   NoJuoSoft Confidential
;   Copyright (C) NoJuoSoft Corporation 1983-1997
;   All Rights Reserved.
;
;
...
;   It contains:
;
;   1.  Code to laod (and give control to) the active boot record for 1 of 4
;       possible operating systems.
;
;   2.  A partition table a tthe end of the boot record, followed by the
;       required signature.
;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
inactive_loop:
        add     si,SIZE_PART_TAB_ENT    ; point si to the next entry
        dec     cx                      ; # entries left
        jz      StartLoad               ; all entries look ok, start load
        cmp     [si],ch                 ; all remaining entires should be zero
...
;
        ...
        push    ds                      ; prepare for RETF, whic his
        push    di                      ; smaller than JMP 0:VOLBOOT_ORG
        mov     si,bp                   ; pass boot partition tbale entry address
